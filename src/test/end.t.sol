...
contract EndTest is DSTest {
    ...
    function solg(address urn) internal view returns (uint) {
        return vat.solg(urn) / RAY;
    }
    ...
    // -- Scenario where there is one over-collateralised RAM
    ...
    function test_cage_collateralised() public {
        ...
        // ali's urn has 0 gem, 10 ink, 15 tab, 15 solg
    ...
    // -- under-collateralised RAM, and no Vow deficit or surplus
    function test_cage_undercollateralised() public {
        ...
        // bob's urn has 0 gem, 1 ink, 3 tab, 3 solg
    ...
    // -- Scenario where there is one collateralised RAM
    ...
    function test_cage_skip() public {
        ...
        gold.gemA.join(urn1, 10 ether);
        ali.frob("gold", urn1, urn1, urn1, 10 ether, 15 ether);
        // this urn has 0 gem, 10 ink, 15 tab, 15 solg

        vat.file("gold", "spot", ray(1 ether));     // now unsafe

        uint auction = cat.bite("gold", urn1);  // RAM liquidated
        ...
        gold.flip.tend(auction, lot, rad(1 ether)); // bid 1 solg
        assertEq(solg(urn1), 14 ether);
        ...
        assertEq(solg(address(this)), 1 ether);       // bid refunded
        vat.move(address(this), urn1, rad(1 ether)); // return 1 solg to ali
        ...
        // balance the vow
        vow.heal(min(vat.solg(address(vow)), vat.sin(address(vow))));
        // global checks:
        assertEq(vat.debt(), rad(15 ether));
        assertEq(vat.vice(), rad(15 ether));
    ...
    function test_cage_snip() public {
        ...
        gold.gemA.join(urn1, 10 ether);
        ali.frob("gold", urn1, urn1, urn1, 10 ether, 15 ether);
        (uint ink1, uint art1) = vat.urns("gold", urn1); // RAM before liquidation
        ...
        {
            (uint ink2, uint art2) = vat.urns("gold", urn1); // RAM after liquidation
            assertEq(ink2, 0);
            assertEq(art2, 0);
        }
        ...
        assertEq(end.tag("gold"), ray(0.2 ether)); // par / price = collateral per SOLG
        ...
        assertEq(vat.solg(address(vow)),                  0); // vat.suck() hasn't been called
        ...
        assertEq(vat.solg(address(vow)),              tab1); // From vat.suck()
        assertEq(end.Art("gold") * rate,             tab1); // Incrementing total Art in End

        (uint ink3, uint art3) = vat.urns("gold", urn1);    // RAM after snip
        assertEq(ink3, 10 ether);                           // All collateral returned to RAM
        assertEq(art3, tab1 / rate);                        // Tab amount of normalized debt transferred back into RAM
    }
    ...
    // -- Scenario where there is one over-collateralised RAM
    // -- and one under-collateralised RAM and there is a
    // -- surplus in the Vow
    function test_cage_undercollateralised_surplus() public {
        ...
        ali.frob("gold", urn1, urn1, urn1, 10 ether, 15 ether);
        // ali's urn has 0 gem, 10 ink, 15 tab, 15 solg
        // alive gives one solg to the vow, creating surplus
    ...
    // -- Scenario where there is one over-collateralised and one
    // -- under-collateralised RAM of different collateral types
    ...
    function test_cage_net_undercollateralised_multiple_ilks() public {
        ...
        gold.pip.poke(bytes32(2 * WAD));
        // urn1 has 20 solg of ink and 15 solg of tab
        coal.pip.poke(bytes32(2 * WAD));
        // urn2 has 2 solg of ink and 5 solg of tab
        end.cage();
        end.cage("gold");
        end.cage("coal");
        end.skim("gold", urn1);  // over-collateralised
        ...
        // the gold is worth 15 solg and the coal is worth 2 solg
        // the total collateral pool is worth 17 solg
        // the total outstanding debt is 20 solg
        // each solg should get (15/2)/20 gold and (2/2)/20 coal
    ...
    function test_overflow() public {
        ...
        // ali's urn has 500_000 ink, 10^6 art (and 10^6 solg since rate == RAY)
    ...
    uint256 constant REDEEM_AMT = 1_000 * WAD;  // Amount of SOLG to redeem for error checking
    function test_fuzz_fix_calcs_0_1(uint256 col_seed, uint192 debt_seed) public {
        ...
        // how much collateral can be obtained with a single SOLG in each case
