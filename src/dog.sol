...
/// dog.sol -- Solg liquidation module 2.0

// Copyright (C) 2020-2022 Solg Foundation
//
...
contract Dog {
    ...
    struct Ilk {
        ...
        uint256 hole;  // Max SOLG needed to cover debt+fees of active auctions per ilk [rad]
        uint256 dirt;  // Amt SOLG needed to cover debt+fees of active auctions per ilk [rad]
    }

    VatLike immutable public vat;  // RAM Engine

    mapping (bytes32 => Ilk) public ilks;

    VowLike public vow;   // Debt Engine
    uint256 public live;  // Active Flag
    uint256 public Hole;  // Max SOLG needed to cover debt+fees of active auctions [rad]
    uint256 public Dirt;  // Amt SOLG needed to cover debt+fees of active auctions [rad]
    ...
    // --- RAM Liquidation: all bark and no bite ---
    //
    // Liquidate a Vault and start a Dutch auction to sell its collateral for SOLG.
    //
    // The third argument is the address that will receive the liquidation reward, if any.
    //
    // The entire Vault will be liquidated except when the target amount of SOLG to be raised in
    ...
    // outstanding SOLG target. The one exception is if the resulting auction would likely
        ...
        uint256 dust;
        {
            ...
            if (art > dart) {
                if (mul(art - dart, rate) < dust) {
                    ...
                    // the extra amount of target SOLG over the limits intended is not of economic concern.
                    dart = art;
                } else {
