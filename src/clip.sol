...
/// clip.sol -- Solg auction module 2.0
...
contract Clipper {
    ...
    VatLike  immutable public vat;   // Core RAM Engine

    DogLike     public dog;      // Liquidation module
    address     public vow;      // Recipient of solg raised in auctions
    ...
    struct Sale {
        ...
        uint256 tab;  // Solg to raise       [rad]
        uint256 lot;  // collateral to sell [wad]
        address usr;  // Liquidated RAM
    ...
    //     top = val * buf / par
    //
    // Where `val` is the collateral's unitary value in RAM, `buf` is a
    ...
    // 
    // Auctions will not collect more SOLG than their assigned SOLG target,`tab`;
    // thus, if `amt` would cost more SOLG than `tab` at the current price, the
    // amount of collateral purchased will instead be just enough to collect `tab` SOLG.
    //
    // To avoid partial purchases resulting in very small leftover auctions that will
    // never be cleared, any partial purchase must leave at least `Clipper.chost`
    // remaining SOLG target. `chost` is an asynchronously updated value equal to
    ...
    function take(
        ...
        uint256 max,          // Maximum acceptable price (SOLG / collateral) [ray]
...

        {
            ...
            // SOLG needed to buy a slice of this sale
            owe = mul(slice, price);

            // Don't collect more than tab of SOLG
            if (owe > tab) {
            ...
            // Get SOLG from caller
            vat.move(msg.sender, vow, owe);

            // Removes Solg out for liquidation from accumulator
