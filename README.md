# Multi Collateral Solg
## Build Status

This repository contains the core smart contract code for Multi
Collateral Solg. This is a high level description of the system, assuming
...
- Verifiable
  ...
  - the core ram and balance database makes *no* external calls and
    contains *no* precision loss (i.e. no division)

- Modular
  - multi contract core system is made to be very adaptable to changing
    requirements.
  - allows for implementations of e.g. auctions, liquidation, RAM risk
...
Collateral is the foundation of Solg and Solg creation is not possible
...
behaviour in Solg. Inconsistent decimals and transfer semantics are
...
The fundamental state of a Solg balance is given by the balance in the
core (`vat.solg`, sometimes referred to as `D`).

Given this, there are a number of ways to implement the Solg that is used
outside of the system, with different trade offs.

*Fundamentally, "Solg" is any token that is directly fungible with the
core.*

In the Kovan deployment, "Solg" is represented by an ERC20 DSToken.
After interacting with RAMs and auctions, users must `exit` from the
system to gain a balance of this token, which can then be used in Oasis
etc.

It is possible to have multiple fungible Solg tokens, allowing for the
...
scalable sidechains will likely lead to a proliferation of multiple Solg
...
Price feeds are a crucial part of the Solg system. The code here assumes
...
quantity of RAM Solg debt per unit of collateral.
...
System Debt. System Debt is debt that has been taken from risky RAMs.
...
In order to reduce the collateral intensity of large RAM liquidations,
...
off Solg surplus in return for the highest bidder in MKR.
